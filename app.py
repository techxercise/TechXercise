...
def draw_styled_landmarks_squat(image, results):
...
def bicepCurlMonitor(camera=False):
    # Right Arm
...
def squatsMonitor(camera=False):
    # Squats
    ...
    # Set mediapipe model 
        if camera == True:
            ...
            while cap.isOpened():
                ...
                # Draw landmarks
                draw_styled_landmarks_squat(image, results)
...
def flamingoMonitor(camera=False):
    # Flamingo
...
def splitsMonitor(camera=False):
    # Splits
...
def index():
    bicepCurlMonitor(False)
    squatsMonitor(False)
    flamingoMonitor(False)
    splitsMonitor(False)
...
def launchSquatsMonitor():
    return render_template('squatsmonitor.html')

@app.route('/flamingomonitor.html')
def launchFlamingoMonitor():
    return render_template('flamingomonitor.html')

@app.route('/splitsmonitor.html')
def launchSplitsMonitor():
    return render_template('splitsmonitor.html')

@app.route('/bicepcurlvideo')
def bicepCurlVideo():
    return Response(bicepCurlMonitor(True), mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/squatsvideo')
def squatsVideo():
    return Response(squatsMonitor(True), mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/flamingovideo')
def flamingoVideo():
    return Response(flamingoMonitor(True), mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/splitsvideo')
def splitsVideo():
    return Response(splitsMonitor(True), mimetype='multipart/x-mixed-replace; boundary=frame')
